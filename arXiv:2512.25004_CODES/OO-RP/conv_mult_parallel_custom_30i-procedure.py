...
test_nr = 'RPOO_H6_4A_30i-procedure_stage01_parallel_NR_003'
...
file_directory = r'/home/jonas/Documents/03_dplts/paper_writing/calculations/codes/random_pos_optimized_operator__CPU_threaded_1.0/random_pos_optimized_operator__CPU_threaded_2.0/adaptvqe/algorithms/stored_adapt-vqe_runs/'
##############################################################
...
def save_data(prefix, append):
    ...
    with open("/home/jonas/Documents/03_dplts/paper_writing/calculations/codes/random_pos_optimized_operator__CPU_threaded_1.0/random_pos_optimized_operator__CPU_threaded_2.0/RPOO_H6_4A_30i-procedure_stage01_parallel_NR_003_insert.pkl", "wb") as f:
        cp.dump(append, f)

def load_data(prefix):
    with open("/home/jonas/Documents/03_dplts/paper_writing/calculations/codes/random_pos_optimized_operator__CPU_threaded_1.0/random_pos_optimized_operator__CPU_threaded_2.0/RPOO_H6_4A_30i-procedure_stage01_parallel_NR_003_insert.pkl", "rb") as f:
        append = cp.load(f)
    return append

def load_data_gr(path):
    with open(path, "rb") as f:
        append = cp.load(f)
    return append

def main():
    if (os.path.exists("/home/jonas/Documents/03_dplts/paper_writing/calculations/codes/random_pos_optimized_operator__CPU_threaded_1.0/random_pos_optimized_operator__CPU_threaded_2.0/RPOO_H6_4A_30i-procedure_stage01_parallel_NR_003_insert.pkl")):
        dictionary = load_data(test_nr)
        print('A')
    else:
        # Falls nicht vorhanden, führe run_adapt aus und speichere die Ergebnisse
        set_settings()
        data_output = run_parallel_measurements(measurement_random, n=len(file_names_old_data), use_processes=False, max_workers=6, file_names_old_data=file_names_old_data)
    ...
    #######################################################
    # import gradient based calculation output
    path_var_prepending = "/home/jonas/Documents/03_dplts/paper_writing/calculations/codes/random_pos_optimized_operator__CPU_threaded_1.0/random_pos_optimized_operator__CPU_threaded_2.0/RPOO_H6_4A_30i-procedure_stage01_parallel_NR_003_insert.pkl"
    if (os.path.exists(path_var_prepending)):
        dictionary_gr = load_data_gr(path_var_prepending)
    else:
        print('False import path for prepending data!')
    ...
    for index, wert in enumerate(dictionary["data_evolution_gradient_norms"]):
        ...
        ###########################################################
        gradient_norms_grad_bas = dictionary_gr["data_evolution_gradient_norms"][index]
        errors_grad_bas = dictionary_gr["data_evolution_errors"][index]
        ###########################################################
        ...
        ax2 = ax1.twinx()
        ax2.plot(iterations, errors_grad_bas[:len(gradient_norms)], label="Gradient Norm (select_via_gradient)", color="grey")
