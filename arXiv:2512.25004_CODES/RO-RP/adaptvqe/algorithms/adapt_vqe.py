...
Both the position of the new ansatz element and the new ansatz element itself are chosen randomly. (https://arxiv.org/abs/2512.25004)
...
class AdaptVQE(metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        if previous_data is not None:
            ...
            self.gradients_backup = previous_data[1]
    ...
    def save_data(self, filename, folder="stored_adapt-vqe_runs"):
        ...
        folder = r"/home/jonas/Documents/03_dplts/paper_writing/calculations/codes/random_operator_random_pos_CPU_threaded_1.0/adaptvqe/algorithms/stored_adapt-vqe_runs/"
    ...
    def rank_gradients(self, coefficients=None, indices=None, silent=False):
        ...
        # select operator and the position where to add the operator on a random basis
        max_operator_index = [random.randint(0, self.pool.size-1)]
        self.operator_pos_arr = [random.randint(0, len(self.indices))] #to allow it to select the max operator position for the best appending gradient

        # cut coefficients and indices at correct operator position in order to add the new operator in the next steps
        coeff_pos_cut_left = coefficients[:self.operator_pos_arr[0]]
        coeff_pos_cut_right = coefficients[self.operator_pos_arr[0]:]
        index_pos_cut_left = indices[:self.operator_pos_arr[0]]
        index_pos_cut_right = indices[self.operator_pos_arr[0]:]

        # calculate and penalize gradient for the selected operator at the selected position
        gradient_calc = self.estimate_gradient(self.operator_pos_arr[0], coefficients = coeff_pos_cut_left + [0] + coeff_pos_cut_right, indices = np.concatenate((index_pos_cut_left,max_operator_index,index_pos_cut_right)).astype('int'), orb_params=None)
        gradient_calc_pen = self.penalize_gradient(gradient_calc, self.operator_pos_arr, silent)
        sel_indices_posx, sel_gradients_posx, total_norm, max_norm = self.rank_gradients_any_pos(self.operator_pos_arr[0], coefficients=self.coefficients, indices=self.indices, silent=False)
        
        # return the selected operator index and its penalized gradient, as well as the total and max gradient norms for the position to the calling function
        return max_operator_index, [gradient_calc_pen], total_norm, max_norm
    ...
    def run(self):
        ...
        if not finished:
            ...
            print('YYYYYYYYYYYYYYYYYYYYXXXXXXXXXXXXXXXXXXYYYYYYYYYYYYYYYYYYYYYYYYYY')
    ...
    def start_iteration(self):
        ...
        print('XXXXXXXXXXXXXXXXXXXYYYYYYYYYYYYYYYYYYYYYYYXXXXXXXXXXXXXXXXXXXXXXXXXXX')
    ...
    def select_via_gradient(self, indices, gradients):
        ...
        # this is only for appending and prepending and not for specifically defined operator positions - thus this section is deactivated here
        if True == False:
            if True == False:
                print('POTENTIAL ERROR: The length of operator_pos_array has been exceeded.')
            if self.pre_app_setting == 'p':
        ...
        # insert the new operator and its data at the specified position within the ansatz
        if True == True:
        ...
        self.gradients_backup = self.gradients
    ...
    def full_optim(
        ...
        # for the problem that g0 is not properly defined if we add in a different position
        g0 = self.estimate_gradients(deepcopy(self.coefficients), deepcopy(self.indices))
