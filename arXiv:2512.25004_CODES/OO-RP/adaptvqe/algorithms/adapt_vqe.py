...
The ansatz position is selected randomly, and the gradients of all pool operators are measured in this position.
We select the operator with the largest energy gradient magnitude for this position. (https://arxiv.org/abs/2512.25004)
...
class AdaptVQE(metaclass=abc.ABCMeta):
    ...
    def save_data(self, filename, folder="stored_adapt-vqe_runs"):
        ...
        folder = r"/home/jonas/Documents/03_dplts/paper_writing/calculations/codes/random_pos_optimized_operator__CPU_threaded_1.0/random_pos_optimized_operator__CPU_threaded_2.0/adaptvqe/algorithms/stored_adapt-vqe_runs/"
    ...
    def rank_gradients_any_pos(self, index_inv, coefficients=None, indices=None, silent=False):
        ...
        for index in range(self.pool.size):
            ...
            if coefficients is not None:
                # calculate gradient at the selected position
                ...
                #gradient = self.penalize_gradient(gradient, index, silent) - no penalization for now
            else:
    ...
    def rank_gradients(self, coefficients=None, indices=None, silent=False):
        ...
        if (self.pre_app_setting == 'a' or self.pre_app_setting == 'p') and (len(self.indices) >= 0):
            ...
            # select operator position on a random manner
            max_operator_position = random.randint(0, len(self.indices))
            self.operator_pos_arr = [max_operator_position]
            
            # optimize within this randomly selected position
            sel_indices, sel_gradients, total_norm, max_norm = self.rank_gradients_any_pos(max_operator_position, coefficients=coefficients, indices=indices, silent=True)

            # return the data
            return sel_indices, sel_gradients, total_norm, max_norm
        else:
            # Raise error if if condition is not fulfilled
            print('ERROR: rank_gradients did not enter the if loop required for inserting in a different position.')
            exit()
