...
@author: jonas
@autor original code: mafalda

Short description:
RO/OP (random operator optimized position)
The energy gradient for a fixed number of pool operators is measured for all possible ansatz positions. Here we choose this number to be $10$.
The $10$ operators whose gradients across the ansatz we choose to measure are those with the highest gradient for appending. The selected position
is the one where these gradients are highest. The operator to be added is selected randomly and placed into the identified position. This protocol
should be seen as a control for the OO/OP protocol that we use to understand how important it is to select the highest gradient operator, as
compared to a random operator, when the position is selected via our heuristics. As compared to OO/OP, RO/OP selects the operator from the
complete pool, rather than being restricted to the top 10 operators for appending in a specific iteration. (https://arxiv.org/abs/2512.25004)

IMPORTANT: Do only use the selection criterion via 'gradient' together with this implementation of the code, as we cannot guarantee
that the other selection criteria are properly implemented for different operator positions yet, as this was not yet part of this project.
"""
...
class AdaptVQE(metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        track_prep_g=False,
        previous_data=None,
        pre_app_setting=None,
        operator_pos_arr=None,
        storage_for_names=None
    ):
        """
        Arguments:
            ...
            pre_app_setting (string): select for prepending or appending - must be set in current implementation - instead an error occurs
            operator_pos_arr (list): defines the position, in which the next len(operator_pos_arr) operators should be appended into the ansatz (see https://arxiv.org/abs/2512.25004);
                after this number of iterations, the code does prepending or appending according to pre_app_setting;
            storage_for_names (str): string to define parts of the output file name for storage purposes of the calculation
        """
        ...
        self.pre_app_setting = pre_app_setting
        self.operator_pos_arr = operator_pos_arr
        self.pos_gradient_arr = []
        self.storage = storage_for_names
        ...
        if previous_data is not None:
            self.data = previous_data[0]  # AdaptData object
        ...
        if previous_data is not None:
            self.gradients = previous_data[1]
            print(f'Loaded gradient list: {self.gradients}')
        else:
            print('Error: Problem with importing self.gradients from previous_data.')
    ...
    def save_data(self, filename, folder="stored_adapt-vqe_runs"):
        ...
        """
        # Enter your desired folder path to store the detailed data of the calculation. If you do not provide a correct filepath, an error will occur and the data won't be stored!
        folder = r"/home/jonas/Documents/03_dplts/paper_writing/calculations/codes/optimized_position_random_operator_10operators_v1.0/adaptvqe/algorithms/stored_adapt-vqe_runs/"
        filepath = os.path.join(folder, filename)  # Pfad zur Datei erstellen
        with open(filepath, "wb") as f:
            pickle.dump([self.data,self.gradients], f)
    ...
    def verify_inputs(self):
        ...
        if self.data is not None:
            self.max_adapt_iter = self.max_adapt_iter+self.data.iteration_counter
    ...
    def initialize_with_previous_data(self):
        ...
        self.file_name = str(self.storage+"_ni_"+str(self.data.iteration_counter+self.max_adapt_iter))
    ...
    def load(self, previous_data=None, eig_decomp=None):
        ...
        if previous_data is not None:
            ...
            #defining the length of the previous ansatz in order to not disturb it
            if self.data is not None:
                self.previous_ansatz_length = len(self.coefficients)
                print("ISNOTNON")
            if self.data is None:
                self.previous_ansatz_length = 0
                print("ISNON")
    ...
    def rank_gradients_any_pos(self, index_inv, coefficients=None, indices=None, silent=False):
        """
        ...
        for index in range(self.pool.size):
            ...
            if coefficients is not None:
                # cut coefficients and indices at correct operator position
                coeff_pos_cut_left = coefficients[:index_inv]
                coeff_pos_cut_right = coefficients[index_inv:]
                index_pos_cut_left = indices[:index_inv]
                index_pos_cut_right = indices[index_inv:]
                gradient = self.estimate_gradient(index_inv, coefficients = coeff_pos_cut_left + [0] + coeff_pos_cut_right, indices = np.concatenate((index_pos_cut_left,[index],index_pos_cut_right)).astype('int'))
                gradient = self.penalize_gradient(gradient, index, silent)
            else:
                gradient = self.eval_candidate_gradient(index, coefficients, indices)

            gradient = self.penalize_gradient(gradient, index, silent) #- currently no penalization activated
    ...
    def rank_gradients(self, coefficients=None, indices=None, silent=False):
        ...
        for index in range(self.pool.size):
            if self.operator_pos_arr is None or True:
                if self.operator_pos_arr is not None and True:
                    #print('Operatorarray is not none or true!')
                    pass
                if self.pre_app_setting == 'p':
                        gradient = self.eval_candidate_gradient_prepending(index)
                if self.pre_app_setting == 'a':
                        
                        gradient = self.eval_candidate_gradient(index, coefficients, indices)
                        #gradient_prepending.append(self.eval_candidate_gradient_prepending(index))
                        gradient_appending.append(gradient)
                if (self.pre_app_setting !='p') and (self.pre_app_setting !='a'):
                    print('ERROR: Prepending or appending setting is incorrect. Please enter a or p as string type for the settings to the variable self.pre_app_setting. a ... appending; p ... prepending')
            ...
            # Find the index of the 10 best operators continuously in the ordered gradient list
            sel_gradients, sel_indices = self.place_gradient(
                gradient, index, sel_gradients, sel_indices, True
            )

            if index not in self.pool.parent_range:
                total_norm += gradient**2

        if (self.pre_app_setting == 'a' or self.pre_app_setting == 'p') and (len(self.indices) >= 0):
            
            # selecting a random operator which is not in the below_threshold list
            candidates_for_random_selection = [i for i in range(self.pool.size) if i not in below_threshold]
            selected_operator_for_interting = ([random.choice(candidates_for_random_selection)] if candidates_for_random_selection else [])

            # take the 10 highest appending operators and store them into fused_index_list for further processing
            fused_index_list = sel_indices

            # define the arrays for storing all gradients and indices for all operator positions during the next steps
            all_pos_gradient_arr = []
            all_pos_index_arr = []

            # define the default max operator index, gradient and position for starting the iterations in the next step
            max_operator_index = sel_indices[0] if sel_indices else []
            max_operator_gradient = 0
            max_operator_position = 0

            # identify the maximum gradient over all operator positions for all operators in fused_index_list and store position and operator
            for operator_position in range(len(self.indices)+1):
                gradient_array = []
                index_arr = []
                for i in range(len(fused_index_list)):
                    # cut coefficients and indices at correct operator position
                    coeff_pos_cut_left = (coefficients)[:operator_position]
                    coeff_pos_cut_right = (coefficients)[operator_position:]
                    index_pos_cut_left = (indices)[:operator_position]
                    index_pos_cut_right = (indices)[operator_position:]

                    # calculate the gradient of position operator position and gradient with index i
                    gradient_calc = self.estimate_gradient(operator_position, coefficients = coeff_pos_cut_left + [0] + coeff_pos_cut_right, indices = np.concatenate((index_pos_cut_left,[fused_index_list[i]],index_pos_cut_right)).astype('int'), orb_params=None)
                    gradient_calc_pen = self.penalize_gradient(gradient_calc, fused_index_list[i], silent)

                    # store gradient and index for eventual use for further research and inspection of them
                    gradient_array.append(gradient_calc_pen)
                    index_arr.append(fused_index_list[i])

                    # check whether the gradient is the largest of with respect to its norm encountered so far - if yes overwrite previous max values
                    if np.abs(gradient_calc_pen) > np.abs(max_operator_gradient):
                        max_operator_gradient = gradient_calc_pen
                        max_operator_index = fused_index_list[i]
                        if [max_operator_index] != [fused_index_list[i]]:
                            print('ERROR: Indices do not match!!!')
                            exit()
                        max_operator_position = operator_position

                # store gradient and index arrays for eventual use for further research and inspection of them
                all_pos_gradient_arr.append(gradient_array)
                all_pos_index_arr.append(index_arr)

            # define position for adding the new operators in self.operator_pos_arr, which is then used by select via gradient, ...
            self.operator_pos_arr = [max_operator_position] #to allow it to select the max operator position for the best appending gradient
            print(self.operator_pos_arr)
            
            # calculate the total norm and maximum norm for the selected operator position
            sel_indices, sel_gradients, total_norm, max_norm = self.rank_gradients_any_pos(max_operator_position, coefficients=self.coefficients, indices=self.indices, silent=True)

            # estimate gradient of the randomly selected operator which will be added
            gradient_of_selected_operator = self.estimate_gradient(max_operator_position, coefficients = coeff_pos_cut_left + [0] + coeff_pos_cut_right, indices = np.concatenate((index_pos_cut_left,selected_operator_for_interting,index_pos_cut_right)).astype('int'), orb_params=None)
            gradient_of_selected_operator_pen = self.penalize_gradient(gradient_of_selected_operator, max_operator_position, silent) #- currently no penalization activated

            # return the values to the calling function
            return selected_operator_for_interting, [gradient_of_selected_operator_pen], total_norm, max_norm


        print(f'Gradient Arr: {self.pos_gradient_arr}')
        total_norm = np.sqrt(total_norm)
    ...
    def place_gradient(self, gradient, index, sel_gradients, sel_indices, deactivate_direct_decimation_to_candidate_number=False):
        """
        ...
        if i < self.window:
            ...
            if deactivate_direct_decimation_to_candidate_number == False:
                sel_indices = sel_indices[:i] + [index] + sel_indices[i : self.window - 1]
                sel_gradients = (
                sel_gradients[:i] + [gradient] + sel_gradients[i : self.window - 1]
                )
        
        if deactivate_direct_decimation_to_candidate_number == True:
            sel_indices = sel_indices[:i] + [index] + sel_indices[i:] # returns the 10 highest gradient operators - for rank gradients optimized optimized
            sel_indices = sel_indices[:10]
            sel_gradients = (
            sel_gradients[:i] + [gradient] + sel_gradients[i:]
            )
            sel_gradients = sel_gradients[:10]
    ...
    def eval_candidate_gradients_prepending(
        ...
        for index in range(self.pool.size):
            ...
            norm += gradient**2
            prep_gradients_a.append(self.penalize_gradient(gradient, index, True))
    ...
    def run(self):
        ...
        if finished:
            ...
            self.save_data(filename=f"{self.file_name}.pkl")
            print(
    ...
    def update_viable_candidates(self, viable_candidates, viable_gradients):
        ...
        if self.tetris:
            ...
            if True == False:
                if True == False:
                    print('POTENTIAL ERROR: The length of operator_pos_array has been exceeded.')
                if self.pre_app_setting == 'p':
            ...
            if True == True:
                new_qubits = self.pool.get_qubits(
                    self.indices[self.operator_pos_arr[0]]
                    )  # Qubits acted on non-trivially by last operator added
            if not ((isinstance(self.operator_pos_arr, (list, np.ndarray)) and 0 > (len(self.operator_pos_arr) if self.operator_pos_arr is not None else 0)) or (True == False)):
                print(f'ERROR: The data type of the operator_pos_arr is not valid. Please recheck your input.')
        ...
        if viable_candidates and self.sel_criterion in ["line_search", "energy"]:
            ...
            if True == False:
                ...
                if self.pre_app_setting == 'p':
                    viable_gradients = [
                        self.estimate_gradient(0, [0] + self.coefficients, [c] + self.indices)
                ...
                if (self.pre_app_setting !='p') and (self.pre_app_setting !='a'):
                    print('ERROR: Prepending or appending setting is incorrect. Please enter a or p as string type for the settings to the variable self.pre_app_setting. a ... appending; p ... prepending')
            if True == True:
                coeff_pos_cut_left = self.coefficients[:self.operator_pos_arr[0]]
                coeff_pos_cut_right = self.coefficients[self.operator_pos_arr[0]:]
                index_pos_cut_left = self.indices[:self.operator_pos_arr[0]]
                index_pos_cut_right = self.indices[self.operator_pos_arr[0]:]
                viable_gradients = [
                        self.estimate_gradient(self.operator_pos_arr[0], np.concatenate((coeff_pos_cut_left,[0],coeff_pos_cut_right)).astype('float'), np.concatenate((index_pos_cut_left,[c],index_pos_cut_right)).astype('float'))
                        for c in viable_candidates
                    ]
            if not ((isinstance(self.operator_pos_arr, (list, np.ndarray)) and 0 > (len(self.operator_pos_arr) if self.operator_pos_arr is not None else 0)) or (True == False)):
                print(f'ERROR: The data type of the operator_pos_arr is not valid. Please recheck your input.')
    ...
    def converged(self):
        ...
        #)
        self.file_name = str(self.storage+"_ni_"+str(self.data.iteration_counter))
    ...
    def inter_it_bfgs_update(self):
        ...
        if True == False:
            if True == False:
                print('POTENTIAL ERROR: The length of operator_pos_array has been exceeded.')
            if self.pre_app_setting == 'p':
                xk = [0] + self.old_coefficients
                gk = self.estimate_gradient(operator_pos=0, coefficients=xk, indices=self.indices) + self.old_gradients
            if self.pre_app_setting == 'a':
        ...
        if True == True:
            coeff_pos_cut_left = self.coefficients[:self.operator_pos_arr[0]]
            coeff_pos_cut_right = self.coefficients[self.operator_pos_arr[0]:]
            gradients_pos_cut_left = self.old_gradients[:self.operator_pos_arr[0]]
            gradients_pos_cut_right = self.old_gradients[self.operator_pos_arr[0]:]
            xk = np.concatenate((coeff_pos_cut_left,[0],coeff_pos_cut_right)).astype('float')
            gk = np.concatenate((gradients_pos_cut_left,[self.estimate_gradient(operator_pos=self.operator_pos_arr[0], coefficients=xk, indices=self.indices)],gradients_pos_cut_right)).astype('float')
        if not ((isinstance(self.operator_pos_arr, (list, np.ndarray)) and 0 > (len(self.operator_pos_arr) if self.operator_pos_arr is not None else 0)) or (True == False)):
            print(f'ERROR: The data type of the operator_pos_arr is not valid. Please recheck your input.')
    ...
    def partial_optim(self, gradient):
        ...
        if True == False:
            if True == False:
                print('POTENTIAL ERROR: The length of operator_pos_array has been exceeded.')
            if self.pre_app_setting == 'p':
                self.coefficients[0] = coef
                self.gradients[0] = new_gradient
            if self.pre_app_setting == 'a':
            ...
            if (self.pre_app_setting !='p') and (self.pre_app_setting !='a'):
                print('ERROR: Prepending or appending setting is incorrect. Please enter a or p as string type for the settings to the variable self.pre_app_setting. a ... appending; p ... prepending')
        if True == True:
            self.coefficients[self.operator_pos_arr[0]] = coef
            self.gradients[self.operator_pos_arr[0]] = new_gradient
        if not ((isinstance(self.operator_pos_arr, (list, np.ndarray)) and 0 > (len(self.operator_pos_arr) if self.operator_pos_arr is not None else 0)) or (True == False)):
            print(f'ERROR: The data type of the operator_pos_arr is not valid. Please recheck your input.')
    ...
    def get_quad_fit_minimizers(self, indices, gradients):
        ...
        for index, gradient in zip(indices, gradients):
            ...
            if True == False:
                ...
                if self.pre_app_setting == 'p':
                    snd_der = self.estimate_snd_derivative(
                        0, #Assume operators would go to the beginning of the ansatz
                        coefficients= [0] + self.coefficients,
                        indices= [index] + self.indices,
                        method="an",
                    )
                if self.pre_app_setting == 'a':
                    snd_der = self.estimate_snd_derivative(
                        len(self.indices), # Assume operators would go to the end of the ansatz
            ...
            if True == True:
                ...
                index_pos_cut_right = self.indices[self.operator_pos_arr[0]:]
                snd_der = self.estimate_snd_derivative(
                        self.operator_pos_arr[0], # Assume operator would go to the position given in operator_pos_array.
                        coefficients=np.concatenate((coeff_pos_cut_left,[0],coeff_pos_cut_right)),
                        indices=np.concatenate((index_pos_cut_left,[index],index_pos_cut_right)),
                        method="an",
                    )
            if not ((isinstance(self.operator_pos_arr, (list, np.ndarray)) and 0 > (len(self.operator_pos_arr) if self.operator_pos_arr is not None else 0)) or (True == False)):
                print(f'ERROR: The data type of the operator_pos_arr is not valid. Please recheck your input.')
            ...
            if True == False:
                if True == False:
                    print('POTENTIAL ERROR: The length of operator_pos_array has been exceeded.')
                if self.pre_app_setting == 'p':
                    printout_a = f"{self.evaluate_energy([minimizer] + self.coefficients, [index] + self.indices)}"
                if self.pre_app_setting == 'a':
                    printout_a = f"{self.evaluate_energy(self.coefficients + [minimizer], self.indices + [index])}"
                if (self.pre_app_setting !='p') and (self.pre_app_setting !='a'):
                    print('ERROR: Prepending or appending setting is incorrect. Please enter a or p as string type for the settings to the variable self.pre_app_setting. a ... appending; p ... prepending')
            if True == True:
                coeff_pos_cut_left = self.coefficients[:self.operator_pos_arr[0]]
                ...
                index_pos_cut_left = self.indices[:self.operator_pos_arr[0]]
                index_pos_cut_right = self.indices[self.operator_pos_arr[0]:]
                printout_a = f"{self.evaluate_energy(np.concatenate((coeff_pos_cut_left,[minimizer],coeff_pos_cut_right)), np.concatenate((index_pos_cut_left,[index],index_pos_cut_right)))}"
            if not ((isinstance(self.operator_pos_arr, (list, np.ndarray)) and 0 > (len(self.operator_pos_arr) if self.operator_pos_arr is not None else 0)) or (True == False)):
                print(f'ERROR: The data type of the operator_pos_arr is not valid. Please recheck your input.')
    ...
    def grow_ansatz(self, viable_candidates, viable_gradients, max_additions=1):
        ...
        while max_additions > 0:
            ...
            if True == False:
                if True == False:
                    print('POTENTIAL ERROR: The length of operator_pos_array has been exceeded.')
                if self.pre_app_setting == 'p':
                    new_indices = self.indices[:len(self.indices) - old_size]
                if self.pre_app_setting == 'a':
                    new_indices = self.indices[old_size:]
                if (self.pre_app_setting !='p') and (self.pre_app_setting !='a'):
                    print('ERROR: Prepending or appending setting is incorrect. Please enter a or p as string type for the settings to the variable self.pre_app_setting. a ... appending; p ... prepending')
            print('YYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY')
            print((len(self.operator_pos_arr) if self.operator_pos_arr is not None else 0))
            print(0 < (len(self.operator_pos_arr) if self.operator_pos_arr is not None else 0))
            print('YYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY')
            if True == True:
                if len(self.data.evolution.indices) != 0:
                    a=self.indices
                    b=self.data.evolution.indices[-1]
                    a, b = a[:], b[:]
                    for elem in b:
                        if elem in a:
                            a.remove(elem)
        ...
        print(f"Operator(s) added to ansatz: {new_indices} (IMPORTANT: The order in which the indices are given here might not be equal to the order in which they are in the ansatz.)")
    ...
    def select_via_1d_quad_fit(self, indices, gradients):
        ...
        if True == False:
            ...
            if (self.pre_app_setting !='p') and (self.pre_app_setting !='a'):
                print('ERROR: Prepending or appending setting is incorrect. Please enter a or p as string type for the settings to the variable self.pre_app_setting. a ... appending; p ... prepending')
        if True == True:
            self.indices.insert(self.operator_pos_arr[0], index)
            self.coefficients.insert(self.operator_pos_arr[0], coefficient)
        if not ((isinstance(self.operator_pos_arr, (list, np.ndarray)) and 0 > (len(self.operator_pos_arr) if self.operator_pos_arr is not None else 0)) or (True == False)):
            print(f'ERROR: The data type of the operator_pos_arr is not valid. Please recheck your input.')
    ...
    def select_via_sampling(self, indices, gradients, probabilities=None):
        ...
        if True == False:
            ...
            if self.pre_app_setting == 'p':
                ...
                self.gradients = np.insert(self.gradients, 0, gradient)
            if self.pre_app_setting == 'a':
                self.indices.append(index)
        ...
        if True == True:
            self.indices.insert(self.operator_pos_arr[0], index)
            self.coefficients.insert(self.operator_pos_arr[0], 0)
            self.gradients = np.insert(self.gradients, self.operator_pos_arr[0], gradient)
        if not ((isinstance(self.operator_pos_arr, (list, np.ndarray)) and 0 > (len(self.operator_pos_arr) if self.operator_pos_arr is not None else 0)) or (True == False)):
            print(f'ERROR: The data type of the operator_pos_arr is not valid. Please recheck your input.')
    ...
    def select_via_gradient(self, indices, gradients):
        ...
        # not to be used for adding in a specific position within the ansatz but only for appending and prepending - thus deactivated here
        if True == False:
            if True == False:
                print('POTENTIAL ERROR: The length of operator_pos_array has been exceeded.')
            if self.pre_app_setting == 'p':
        ...
        # now used for adding the new operator index, coefficient, gradient in a specific position within the ansatz defined by self.operator_pos_arr
        if True == True:
    ...
    def select_via_gradient_ceo(self, indices, gradients):
        ...
        if self.progressive_opt or self.dve:
            # We perform the optimization here for all candidatesestimate_gradient
            energy, gradient, nfevs, ngevs, nits = self.select_via_energy(
    ...
    def add_ceo_candidate(self, candidate, gradient):
        ...
        if True == False:
            ...
            if self.pre_app_setting == 'p':
                self.gradients = np.insert(self.gradients, 0, gradient)
                self.indices = np.insert(indices, 0, candidate)
                self.coefficients = list(np.insert(self.coefficients, 0, [0 for _ in candidate]))
            if self.pre_app_setting == 'a':
                self.gradients = np.append(self.gradients, gradient)
        ...
        if True == True:
            ...
            self.gradients = np.insert(self.gradients, self.operator_pos_arr[0], gradient)
            self.indices = np.concatenate((index_pos_cut_left,[candidate],index_pos_cut_right)).astype('int')
            self.coefficients = np.concatenate((coeff_pos_cut_left,[0 for _ in candidate],coeff_pos_cut_right)).astype('float')
        if not ((isinstance(self.operator_pos_arr, (list, np.ndarray)) and 0 > (len(self.operator_pos_arr) if self.operator_pos_arr is not None else 0)) or (True == False)):
            print(f'ERROR: The data type of the operator_pos_arr is not valid. Please recheck your input.')
        return gradient
    ...
    def select_via_1d_energy(self, indices, gradients):
        ...
        if True == False:
            ...
            if self.pre_app_setting == 'p':
                self.indices.insert(0, index)
                self.coefficients.insert(0, 0) # Could also insert the new optimized coefficient
                self.gradients = np.insert(self.gradients, 0, gradient)
            if self.pre_app_setting == 'a':
                self.indices.append(index)
        ...
        if True == True:
            self.indices.insert(self.operator_pos_arr[0], index)
            self.coefficients.insert(self.operator_pos_arr[0], 0) # Could also insert the new optimized coefficient
            self.gradients = np.insert(self.gradients, self.operator_pos_arr[0], gradient)
        if not ((isinstance(self.operator_pos_arr, (list, np.ndarray)) and 0 > (len(self.operator_pos_arr) if self.operator_pos_arr is not None else 0)) or (True == False)):
            print(f'ERROR: The data type of the operator_pos_arr is not valid. Please recheck your input.')
    ...
    def select_via_energy(self, indices, gradients, maxiters=None):
        ...
        if True == False:
            if True == False:
                print('POTENTIAL ERROR: The length of operator_pos_array has been exceeded.')
            if self.pre_app_setting == 'p':
                self.indices = np.insert(indices, 0, index)
            if self.pre_app_setting == 'a':
                self.indices = np.append(indices, index)
            if (self.pre_app_setting !='p') and (self.pre_app_setting !='a'):
                print('ERROR: Prepending or appending setting is incorrect. Please enter a or p as string type for the settings to the variable self.pre_app_setting. a ... appending; p ... prepending')
        if True == True:
            self.indices = np.insert(indices, self.operator_pos_arr[0], index)
        if not ((isinstance(self.operator_pos_arr, (list, np.ndarray)) and 0 > (len(self.operator_pos_arr) if self.operator_pos_arr is not None else 0)) or (True == False)):
            print(f'ERROR: The data type of the operator_pos_arr is not valid. Please recheck your input.')
    ...
    def prepare_parallel_opt(self, index, gradient, maxiters=None):
        ...
        if True == False:
            if True == False:
                print('POTENTIAL ERROR: The length of operator_pos_array has been exceeded.')
            if self.pre_app_setting == 'p':
                initial_coefficients = [0 for _ in range(len(gradient))] + self.coefficients.copy()
            if self.pre_app_setting == 'a':
        ...
        if True == True:
            coeff_pos_cut_left = self.coefficients[:self.operator_pos_arr[0]]
            coeff_pos_cut_right = self.coefficients[self.operator_pos_arr[0]:]
            initial_coefficients = np.concatenate((coeff_pos_cut_left,[0 for _ in range(len(gradient))],coeff_pos_cut_right)).astype('int')
        if not ((isinstance(self.operator_pos_arr, (list, np.ndarray)) and 0 > (len(self.operator_pos_arr) if self.operator_pos_arr is not None else 0)) or (True == False)):
            print(f'ERROR: The data type of the operator_pos_arr is not valid. Please recheck your input.')
        ...
        if True == False:
            if True == False:
                print('POTENTIAL ERROR: The length of operator_pos_array has been exceeded.')
            if self.pre_app_setting == 'p':
                test_indices = np.insert(test_indices, 0, index)
            if self.pre_app_setting == 'a':
                test_indices = np.append(test_indices, index)
            if (self.pre_app_setting !='p') and (self.pre_app_setting !='a'):
                print('ERROR: Prepending or appending setting is incorrect. Please enter a or p as string type for the settings to the variable self.pre_app_setting. a ... appending; p ... prepending')
        if True == True:
            index_pos_cut_left = test_indices[:self.operator_pos_arr[0]]
            index_pos_cut_right = test_indices[self.operator_pos_arr[0]:]
            test_indices = np.concatenate((index_pos_cut_left,[index],index_pos_cut_right)).astype('int')
        if not ((isinstance(self.operator_pos_arr, (list, np.ndarray)) and 0 > (len(self.operator_pos_arr) if self.operator_pos_arr is not None else 0)) or (True == False)):
            print(f'ERROR: The data type of the operator_pos_arr is not valid. Please recheck your input.')
        ...
        if self.recycle_hessian:
            if True == False:
                if True == False:
                    print('POTENTIAL ERROR: The length of operator_pos_array has been exceeded.')
                if self.pre_app_setting == 'p':
                    g0 = np.insert(self.gradients, 0, gradient)
                if self.pre_app_setting == 'a':
                    g0 = np.append(self.gradients, gradient)
                if (self.pre_app_setting !='p') and (self.pre_app_setting !='a'):
                    print('ERROR: Prepending or appending setting is incorrect. Please enter a or p as string type for the settings to the variable self.pre_app_setting. a ... appending; p ... prepending')
            if True == True:
                g0 = np.insert(self.gradients, self.operator_pos_arr[0], gradient)
            if not ((isinstance(self.operator_pos_arr, (list, np.ndarray)) and 0 > (len(self.operator_pos_arr) if self.operator_pos_arr is not None else 0)) or (True == False)):
                print(f'ERROR: The data type of the operator_pos_arr is not valid. Please recheck your input.')
            e0 = self.energy
        else:
    ...
    def full_optim(
        ...
        if self.recycle_hessian and (not self.data.iteration_counter and self.orb_opt):
            ...
            g0 = self.estimate_gradients(initial_coefficients, indices) #MIGHTNEEDCHANGEFORPREPENDING
            extra_njev = 1
        ...
        if opt_result.nit:
            ...
            # additional loop to solve problems of self.gradients becoming None when nit=0 for operator positions different to appending and prepending
            if g0 is None:
                gradients = self.estimate_gradients(initial_coefficients, indices)
            else:
                gradients = g0
    ...
    def prepare_opt(
        ...
        ) # doesn't need to be reversed for prepending, since it has no effect on it.
    ...
    def minimize_1d(self, index=None, state=None, max_iter=100, e0=None, g0=None):
        ...
        if index is None:
            if True == False:
                ...
                if (self.pre_app_setting !='p') and (self.pre_app_setting !='a'):
                    print('ERROR: Prepending or appending setting is incorrect. Please enter a or p as string type for the settings to the variable self.pre_app_setting. a ... appending; p ... prepending')
            if True == True:
                index = self.indices[self.operator_pos_arr[0]]
            if not ((isinstance(self.operator_pos_arr, (list, np.ndarray)) and 0 > (len(self.operator_pos_arr) if self.operator_pos_arr is not None else 0)) or (True == False)):
                print(f'ERROR: The data type of the operator_pos_arr is not valid. Please recheck your input.')
        ...
        if True == False:
            ...
            if self.pre_app_setting == 'p':
                estimate_gradient = lambda x: self.estimate_gradient(
                    x, 0, indices=[index], coefficients=[0], ref_state=state
                )
            if self.pre_app_setting == 'a':
                estimate_gradient = lambda x: self.estimate_gradient(
        ...
        if True == True:
            estimate_gradient = lambda x: self.estimate_gradient(
                x, self.operator_pos_arr[0], indices=[index], coefficients=[0], ref_state=state
            )
        if not ((isinstance(self.operator_pos_arr, (list, np.ndarray)) and 0 > (len(self.operator_pos_arr) if self.operator_pos_arr is not None else 0)) or (True == False)):
            print(f'ERROR: The data type of the operator_pos_arr is not valid. Please recheck your input.')
    ...
    def bfgs_update(self, gfkp1, gfk, xkp1, xk):
        ...
        Arguments:
            ...
            xkp1 (Union[list,np.ndarray]): the coefficients at the new iterate
