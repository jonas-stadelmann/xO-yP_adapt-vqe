...
The energy gradient for a fixed number of pool operators (here taken to be 10) is measured for all possible ansatz positions.
The 10 operators whose gradients across the ansatz we choose to measure are those with the highest gradient for appending.
Out of the 10 operators and all ansatz positions, we select the combination of operator and position associated with the highest gradient. (https://arxiv.org/abs/2512.25004)
...
class AdaptVQE(metaclass=abc.ABCMeta):
    ...
    def save_data(self, filename, folder="stored_adapt-vqe_runs"):
        ...
        folder = r"/home/jonas/Documents/03_dplts/paper_writing/calculations/codes/optimized_position_app_choice_10operator_correct_v1.0/adaptvqe/algorithms/stored_adapt-vqe_runs/"
    ...
    def rank_gradients_any_pos(self, index_inv, coefficients=None, indices=None, silent=False):
        """
        Selects the operators that currently have the largest gradients for a specific operator position. The number of selected operators depends on
        ...
        Arguments:
            index_inv (int): position in the ansatz where to insert the operator for gradient calculation
            coefficients (list): ansatz coefficients
            indices (list): ansatz indices
        ...
        for index in range(self.pool.size):
            ...
            if coefficients is not None:
                # cut coefficients and indices at the operator position selected for the insertion of the new operator
                ...
                # estimate the gradient for the operator at the selected position
            ...
            # Find the index of the operator in the ordered gradient list (here the top 1 best operator as long as self.candidates == 1)
    ...
    def rank_gradients(self, coefficients=None, indices=None, silent=False):
        ...
        for index in range(self.pool.size):
            ...
            # Find the 10 best operators continuously
            sel_gradients, sel_indices = self.place_gradient(
                gradient, index, sel_gradients, sel_indices, True
            )
            sel_indices_ordered.append(sel_indices)
        ...
        if (self.pre_app_setting == 'a' or self.pre_app_setting == 'p') and (len(self.indices) >= 0):
            ...
            # store the 10 best operators selected through appending (if self.pre_app_setting == 'a') into fused_index_list for further processing
            fused_index_list = sel_indices
            
            # lists to store all gradients and indices for all possible operator positions and operators (for possible further investigations)
            all_pos_gradient_arr = []
            all_pos_index_arr = []

            # define default values for max operator index, gradient and position
            max_operator_index = sel_indices[0] if sel_indices else []
            max_operator_gradient = 0
            max_operator_position = 0

            # iterate over all possible operator positions in the ansatz and over all operators simultaneously to find the maximum gradient and corresponding operator and position
            for operator_position in range(len(self.indices)+1):
                ...
                for i in range(len(fused_index_list)):
                    ...
                    # cut coefficients and indices at the operator position to insert the operator selected for this iteration from fused_index_list
                    ...
                    # evaluate the gradient for the operator at fused_index_list[i] at the currently iterated operator position and penalize the gradient if required
                    ...
                    # store the gradient and index for possible further investigations
                    gradient_array.append(gradient_calc_pen)
                    index_arr.append(fused_index_list[i])

                    # check if the calculated gradient is the maximum one over all previous iterations and if so store the gradient, corresponding operator index and position
                ...
                # store all gradients and indices for the currently iterated operator position for possible further investigations
                all_pos_gradient_arr.append(gradient_array)
                all_pos_index_arr.append(index_arr)

            # store the maximum operator position into self.operator_pos_arr, which defines where to add the gradients in for example select via gradient, ...
            ...
            print(all_pos_gradient_arr)
            print(self.operator_pos_arr)
            self.pos_gradient_arr = self.operator_pos_arr
            
            # calculate the gradient total norm over all operators and the maximum norm for this positon
            ...
            # return the maximum operator index, gradient, total norm and max norm to the calling function
            return [max_operator_index], [max_operator_gradient], total_norm, max_norm
    ...
    def inter_it_bfgs_update(self):
        ...
        if len(gk) != len(self.gradients):
            print("ERROR: Length of old gradients does not match new gradients after BFGS update.")
            exit()
        return extra_ngevs
    ...
    def select_via_gradient(self, indices, gradients):
        ...
        # this one is not necessary to use as we add in different operator positions only for appending + prepending protocol
        if True == False:
            if True == False:
                print('POTENTIAL ERROR: The length of operator_pos_array has been exceeded.')
            if self.pre_app_setting == 'p':
        ...
        # this code section is for adding in an alternative operator position defined in operator_pos_array
        if True == True:
